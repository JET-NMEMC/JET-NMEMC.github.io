...
//修改：grid时，xy可以设置不同的网格数量 kriging.grid = function (polygons, variogram, Xwidth, Ywidth)
...
var kriging = function () {
    ...
    kriging.grid = function (polygons, variogram, Xwidth, Ywidth) {
        ...
        var x = Math.ceil((xlim[1] - xlim[0]) / Xwidth);
        var y = Math.ceil((ylim[1] - ylim[0]) / Ywidth);
    ...
    // kriging.plot = function (canvas, grid, xlim, ylim, colors, valuelist = null) {
        ...
        var n = grid.length;
        var m = grid[0].length;
        var wx = Math.ceil(grid.Xwidth * canvas.width / (xlim[1] - xlim[0]));
        var wy = Math.ceil(grid.Ywidth * canvas.height / (ylim[1] - ylim[0]));
        area_list=[]
        ...
        for (i = 0; i < n; i++)
            for (j = 0; j < m; j++) {
                ...
                x = canvas.width * (i * grid.Xwidth + grid.xlim[0] - xlim[0]) / range[0];
                y = canvas.height * (1 - (j * grid.Ywidth + grid.ylim[0] - ylim[0]) / range[1]);
                ...
                if (colors[0] == 0) {
                    if (grid.zlim[0] == grid.zlim[1]) {
                        ...
                        if (z < 0.0) z = 0.0;
                        if (z > 1.0) z = 1.0;
                        ctx.fillStyle = colors[Math.round(grid[i][j])];
                    }
                } else {
                    if (valuelist) {
                        if (valuelist.length) {
                            for (k = 1; k < valuelist.length; k++) {
                                if (grid[i][j] <= valuelist[k]) {
